...
5. As a maintainer reviewing a PR that touches `harness/tui.py`, I see the screenshot export matrix cover the affected screens at all three terminal sizes in the PR check, so that broken rendering, traceback output, or screen-mount regressions are caught before merge.
...
- Should the screenshot matrix include a fourth size — `80×24`, the historical default — for graceful-degradation evidence? Roadmap §8 calls out only the three larger sizes; including `80×24` would surface layout overflow, but may also force layout compromises that hurt the showcase sizes.
