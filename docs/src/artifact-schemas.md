...
| Robot decision traces | `schema_version` via `DECISION_TRACE_SCHEMA_VERSION` | `src/worldforge/decision_trace.py` and `src/worldforge/schemas/decision_trace.v1.schema.json` | `tests/test_decision_trace.py`, `tests/test_cross_embodiment_decision_evidence.py` | `examples/cross-embodiment-decision-evidence/run.py` and its README | Robotics evidence owner |
